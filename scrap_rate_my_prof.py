...
def main():
    ...
    for art_college_name in list(rmp_arts_school_dict.values()):
        count = 0
        review_df = pd.DataFrame(columns=["prof_id","quality","difficulty","comment"])
        df_tids = pd.read_csv(f"./LIBERAL_ARTS_DATA/{art_college_name}_tid.csv")
        ...
        review_df.to_csv(f"./LIBERAL_ARTS_DATA_REVIEWS/{art_college_name}_reviews.csv")

#gets the professor data at specified schools
